...
**C5 (done -- SLX-C5).** As a reviewer, I want the agent prevented from making a failing test pass by rewriting the test's expected/asserted value to match the code's actual (buggy) output, rather than fixing the code, so that a passing retry always means the original requirement was met -- not quietly redefined.
- *Discovered:* SLX-D2 smoke testing (2026-07-04) -- asked to fix `add(0.1, 0.2) == 0.3` under a local model (qwen2.5-coder:14b), attempt 3 "passed" by rewriting the assertion's literal to `0.30000000000000004` (add()'s actual output) instead of fixing anything. Tests went green; the requirement was never met. Same failure family as the SLX-C1 duplicate-subtract bug.
- *Why it matters:* nothing today distinguishes "the code was fixed" from "the assertion was rewritten to match whatever the code already does" -- a structural gap in the C4 self-correction loop, not a one-off model mistake. The PR body (SLX-D2) does surface the retry and the intermediate failure message, so an attentive reviewer has a chance to catch it, but the pipeline itself has no guardrail.
- *Delivered (SLX-C5, 2026-07-04):* pure-Python detection (`execution.orchestrator.detect_assertion_gaming`) parses the previous attempt's real pytest failure output for the actual/expected values of a failed `assert x == y` and flags a retry diff that swaps the expected literal to the previous actual value -- surfaced via the existing "Needs attention" PR section and a CLI warning, detection-only (not blocking), per the acceptance scoping below. Re-verified end-to-end against real Ollama (qwen2.5-coder:14b): see SLX-C5 smoke test and the C7 finding below.
- *Acceptance (met):* a retry that changes a literal/expected value in a test to exactly the previous attempt's actual (failing) output is flagged distinctly from a retry that only changes implementation code, surfaced as a stronger warning in the PR body ("Needs attention"), without auto-blocking or forcing additional retries.
- *Known scope limit -- see C7:* only catches an exact literal swap. A retry that weakens the assertion itself (`round()`, `pytest.approx`, a hand-rolled tolerance) rather than swapping in a matching literal passes straight through undetected.

**C7 (backlog, not yet scoped).** As a reviewer, I want the agent to flag a retry that makes a failing test pass by weakening the assertion itself -- wrapping it in `round()`, `pytest.approx()`, `math.isclose()`, or a hand-rolled tolerance check -- rather than by fixing the implementation, so that a passing retry can't quietly trade exact correctness for a looser check the original test never asked for.
- *Discovered:* SLX-C5 smoke testing (2026-07-05), while re-verifying C5's exact-literal-swap detector against real Ollama (qwen2.5-coder:14b) on the same `add(0.1, 0.2) == 0.3` case. Across roughly 8 real-model retry attempts under varying prompt framings, the model reached for `round(...)`/`pytest.approx(...)`/`abs(...) < 1e-9`-style tolerance rewrites far more often than the literal-swap pattern C5 was built to catch -- the literal swap only appeared when the prompt explicitly pointed at "update the expected value to match what it actually returns."
- *Why it matters:* this suggests tolerance-weakening, not literal-swapping, is this local model's *default* test-gaming instinct -- meaning SLX-C5's detector, while correct for what it targets, currently misses the more common real-world case. Same underlying harm as C5 (a passing retry that never met the original requirement), different and currently invisible mechanism.
- *Acceptance (draft, needs proper scoping):* a retry diff that wraps a previously-plain equality assertion in `round()`, `pytest.approx()`, `math.isclose()`, or an inline tolerance comparison (e.g. `abs(a - b) < epsilon`) -- where the prior attempt's failure was a plain `==` mismatch on that same assertion -- is flagged distinctly in the PR body, same detection-only pattern as C5 (no auto-blocking). Needs scoping on: how to treat a tolerance that was already present in the original test (not gaming) vs. newly introduced by a retry; whether a human-reasonable epsilon should change the flag's severity; and whether this should share detection plumbing with C5 or be a separate check.

**C8 (backlog, not yet scoped).** As a developer, I want retry-correction prompts to never contain Solvix's own internal scratch-file paths, so that the model isn't confused into thinking a real file is missing when the problem is actually a diff-apply mismatch.
- *Discovered:* real smoke testing (2026-07-05) against `cli.py` (a ~600+ line real file, unlike the small sample_repo fixtures used everywhere else so far). Attempt 3's response refused to produce a diff and instead asked for "the content of the specific file (`target` in this case)" -- `target` is not a real file anywhere in the repo; it's the literal basename `reasoning.editor._validate_applies_cleanly`/`_apply_diff_to_content` give the scratch copy they dry-run `patch` against (`Path(tmp_dir) / "target"`).
- *Why it matters:* `_validate_applies_cleanly` raises `DiffGenerationError(f"diff did not apply cleanly against file content: {result.stdout}{result.stderr}")`, and `patch`'s own stdout/stderr echoes back the literal positional path it was invoked with (e.g. `patching file '/var/folders/.../tmpXXXXXX/target'`). That raw message then reaches the model verbatim through two separate paths: `propose_diff`'s own one-shot self-correction retry (`_CORRECTION_TEMPLATE.format(error=first_error, ...)`, reasoning/editor.py) and, on the next outer attempt, `execution.orchestrator`'s retry loop (`PreviousAttempt(failure_output=str(error))`, embedded into the next prompt's "Test failure output" section by `_build_user_message`). The model has no way to know `target`/the `/tmp/...` path is Solvix's own internal naming, not a file it's missing -- so it can burn a whole attempt asking for context that was never relevant, on top of whatever the real diff-mismatch problem was. Larger, more realistic files (this codebase's own `cli.py`, not sample_repo's small fixtures) make diff-apply mismatches more common in the first place, so this previously invisible confusion is likely to keep showing up as Solvix is used against real repos.
- *Acceptance (draft, needs proper scoping):* neither `_CORRECTION_TEMPLATE`'s `{error}` nor a `PreviousAttempt.failure_output` built from a `DiffGenerationError` should ever contain the scratch directory's absolute path or its internal `target`/`change.diff` filenames -- the prompt should describe the failure (e.g. "hunk N did not match the file's current content") without naming Solvix's own temp-file scaffolding. Needs scoping on: whether to strip/replace the path via a regex over `patch`'s output, or have `_validate_applies_cleanly` construct its own message from `patch`'s parsed complaint rather than passing stdout/stderr through verbatim; and whether the same scrubbing is needed anywhere else `DiffGenerationError`/subprocess output reaches a prompt (e.g. lint-correction's `ruff` output, which is file-path-based too but uses the real step file path, not a scratch one, so likely unaffected).
